...
%><body<%= isSingleViewRendering ? " class=\"cq-reports-singleviewrender\"" : "" %>>
    ...
    <%
        if (!isSingleViewRendering) {
    %><h2 class="<%= switchCss %>"><%= StringEscapeUtils.escapeHtml4(actualTitle) %></h2><%
        }
    %>
<%
    if (description != null) {
...
%>
	...
	<div class="exportcsv">
		<a href="#" id="downloadcsv">Export to CSV</a>
	</div>
	<cq:include path="pageLinkRefresher" resourceType="aetna/reporting/components/pageLinkRefresher"/>

...
if ((host_name == "www.aetna.com")) {
	s_account="aetnaaeprod, aetnaglobal";
}
else if (location.hostname.indexOf('survey.foreseeresults.com') != -1) {
    s_account="aetnaaeprod";
}
else { 
	s_account="aetnaaedev";
}
...
s.linkInternalFilters="javascript:,member.aetna.com,www.aetna.com,dev-aetna-www-com.digitas.com,dev-aetna-cms-cq.digitas.com,www.aetnatools.com"
...
s.useCommerce=true;
s.varUsed="eVar33";
s.eventList="event37,event38,event39"; // Abandon,Success,Error
...
/* clickThruQuality variables */
var i=1;

/* Global Pageload Variables */
s.channel="ae"; // Channel

var previousPage = document.referrer;
previousPage = previousPage.replace(location.protocol + "//" + location.host, "");
if (!previousPage || 0 === previousPage.length){
	s.prop11 = "";
} else {
	s.prop11 = "ae" + previousPage.replace(/\//g, ":"); 
}

s.prop10=document.location.href; // URL
s.prop9="D=pageName"; // Backup pageName
...
if(!s.pageType && !s.pageName){
	...
	if(!pathArray[1] || (pathArray[1] == 'index.html')){
		s.pageName = "ae:router";
	}else{
		s.pageName = "ae:" + s.getPageName();
	}
}
...
// setup DynamicObjectIDs
//s.setupDynamicObjectIDs();
//s.prop16=s.eVar16="D=oid";
//s.prop17=s.eVar17="D=pid";
...
myhier = (myhier == "ae|") ? "ae" : myhier; //remove the trailing | if it is just "ae"
s.hier1 = (myhier == "ae|index") ? "ae" : myhier;
//s.hier1=getHier();

s.events=s.apl(s.events,'event2',',',1); // Page View

if (s.getVisitStart("s_visit")=="1"){
	s.events=s.apl(s.events, 'event1', ',', 1);  // Visit Start
}


if (myhier.split("|").length === 3 || 
		(myhier.indexOf("content") > -1 &&
				myhier.split("|").length === 5)){
...
if(s.prop1)s.eVar1="D=c1";//s.eVar1=s.getValOnce("D=c1",'s_evar1',0);

// Days Since Last Visit
s.prop2=s.getDaysSinceLastVisit('s_lv');
if(s.prop2)s.eVar2="D=c2";//s.eVar2=s.getValOnce("D=c2",'s_evar2',0);

/* External Campaign Tracking & Clickthrough Quality */
if(!s.campaign)
...
for (var a = 1; a <= 75; a++)
{
    if (a === 65)
    {
        ...
        s["prop" + a] && (s["prop" + a] = s["prop" + a].toLowerCase().replace(/^d=/, "D=")), s["eVar" + a] && (s["eVar" + a] = s["eVar" + a].toLowerCase().replace(/^d=/, "D=")), a <= 5 && s["hier" + a] && (s["hier" + a] = s["hier" + a].toLowerCase().replace(/^d=/, "D="));}
for (var b = ["products", "pageName", "channel", "campaign"], a = 0; a < b.length; a++) s[b[a]] && (s[b[a]] = s[b[a]].toLowerCase().replace(/^d=/, "D="));
...
var hierVal="ae"+fl1+fl2+fl3+fl4;
return hierVal;}
...
s.visitorNamespace="aetna"
s.trackingServer="metrics.aetnamedicare.com"
s.trackingServerSecure="smetrics.aetnamedicare.com"
